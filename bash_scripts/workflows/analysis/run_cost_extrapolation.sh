        ...
        if [[ $? -eq 0 ]]; then
            ...
            if [[ "$enable_viz" == "true" ]]; then
                if [[ -z "$plots_directory" ]]; then
                    ...
                    echo "  → Generating PCAC cost extrapolation visualizations..."
                    ...
                    if [[ ! -d "$plots_directory" ]]; then
                        ...
                        execute_python_script "$VIZ_SCRIPT" "visualize_cost_extrapolation_analysis" \
                            --analysis_type pcac \
        ...
        if [[ $? -eq 0 ]]; then
            ...
            if [[ "$enable_viz" == "true" ]]; then
                if [[ -z "$plots_directory" ]]; then
                    ...
                    echo "  → Generating pion cost extrapolation visualizations..."
