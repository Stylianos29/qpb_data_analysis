...
def extract_info_from_gauge_links_filename(line):
    ...
    if match_APE_smearing:
        ...
        APE_smearing_alpha = float(match_APE_smearing.group(2) + '.' + match_APE_smearing.group(3))
    ...
    return beta_value, spatial_lattice_size, temporal_lattice_size, APE_smearing_iterations, APE_smearing_alpha, config_label
        ...
        with open(invert_log_file_full_path, 'r') as file:
            for line in file:
                ...
                if 'Gauge field (raw_32) =' in line:
                    self.beta_value, self.spatial_lattice_size, self.temporal_lattice_size, self.APE_smearing_iterations, self.APE_smearing_alpha, self.config_label = extract_info_from_gauge_links_filename(line)
                ...
                if "Done vector = " in line:
                    self.CG_iterations_list.append(extract_CG_iterations_per_vector_inversion(line))

                if "iterrations msCG converged, t =" in line:
                    self.multi_shift_CG_iterations_list.append(extract_multi_shift_CG_iterations(line))

                # if ", t = " in line:
                # if re.search(',\s*t\s*=\s*', line):
                if re.search('iters, CG converged, res = ', line):                    
                    self.time_per_vector_inversion_list.append(extract_time_per_vector_inversion(line))

                if "CG done, 12 vectors in t = ":
                    self.total_time = extract_trailing_number(line.replace("CG done, 12 vectors", ""))
