...
def effective_mass_of_single_state_non_periodic_correlator(correlator_array, time_index):
    '''Calculates the m parameter of the single-state non-period correlator function: m = log(C(t)/C(t+1)).'''

    return np.log(correlator_array[time_index]/correlator_array[time_index+1])


def amplitude_of_single_state_non_periodic_correlator(correlator_array, effective_mass, time_index):
...
def single_state_doubly_periodic_correlator(t, p):
    ...
    TODO: What about the third parameter?'''
    T = 48
    return p[0]*( np.exp(-p[1]*t) + np.exp(-p[1]*(T-t)) + np.exp(-p[1]*(T+t)) + np.exp(-p[1]*(2*T-t)) )
...
def centered_difference_correlator_derivative(correlator_values_array):
    ...
    return -(1/12)*(- np.roll(correlator_values_array, shift=-2)
