        ...
        for group_keys, group_df in grouped:
            ...
            if grouping_variable:
                ...
                for value in unique_group_values:
                    ...
                    self._plot_group(
                        ax, subgroup, label=str(value), color=color, marker=marker
                    )
                ax.legend(
                    title=grouping_variable.replace("_", " "), loc=legend_location
                )
            else:
                # Individual plot
...
class HDF5Plotter(EnhancedHDF5Analyzer):
    ...
    def __init__(self, hdf5_file_path, default_output_directory=None):
        ...
        if default_output_directory is None:
            ...
            print(f"No output directory specified. Using temporary directory: {self.default_output_directory}")
        else:
    ...
    def create_dataset_dataframe(self, dataset_name, add_time_column=True, time_offset=0, 
        ...
        for dataset_path in dataset_paths:
            ...
            while current_path:
                if current_path in self.parameters_by_group:
                    ...
                    for param_name, param_value in self.parameters_by_group[current_path].items():
            ...
            if flatten_arrays and hasattr(dataset_values, 'shape') and dataset_values.size > 1:
    ...
    def create_multi_dataset_dataframe(self, dataset_names, time_aligned=True, add_time_column=True, 
    ...
    def _merge_value_error_datasets(self, base_name, group_path=None):
        ...
        if "_mean_values" in base_name or "_error_values" in base_name:
            ...
            base_name = base_name.replace("_mean_values", "").replace("_error_values", "")
        ...
        if mean_dataset_name not in self.output_quantities["paths_by_name"]:
            raise ValueError(f"Mean dataset '{mean_dataset_name}' not found in HDF5 file.")
        if error_dataset_name not in self.output_quantities["paths_by_name"]:
            raise ValueError(f"Error dataset '{error_dataset_name}' not found in HDF5 file.")
        ...
        # Find common groups
        if group_path:
            common_groups = [group_path] if group_path in mean_by_group and group_path in error_by_group else []
        else:
            common_groups = [group for group in mean_by_group if group in error_by_group]
        
        if not common_groups:
            raise ValueError(f"No matching groups found for both '{mean_dataset_name}' and '{error_dataset_name}'")
    ...
    def create_merged_value_error_dataframe(self, base_name, add_time_column=True, time_offset=0,
    ...
    def plot_datasets(self, dataset_names, output_directory=None, 
        ...
        if merge_value_error:
            if isinstance(dataset_names, str):
                ...
                dataset_names = [dataset_names.replace("_mean_values", "").replace("_error_values", "")]
            else:
        ...
        for dataset_name in dataset_names:
            ...
            clean_name = dataset_name.replace("_mean_values", "").replace("_error_values", "")
