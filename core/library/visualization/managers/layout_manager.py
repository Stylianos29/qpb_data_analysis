...
class PlotLayoutManager:
    ...
    def create_figure(self, figure_size: Optional[Tuple[float, float]] = None) -> Tuple[Figure, Axes]:
        """
    ...
    def setup_axes_labels(self, 
        ...
        if xaxis_label is None and self.constants:
            xaxis_label = getattr(self.constants, 'AXES_LABELS_BY_COLUMN_NAME', {}).get(x_variable, x_variable)
        elif xaxis_label is None:
            xaxis_label = x_variable
            
        # Determine y-axis label
        if yaxis_label is None and self.constants:
            yaxis_label = getattr(self.constants, 'AXES_LABELS_BY_COLUMN_NAME', {}).get(y_variable, y_variable)
    ...
    def setup_axes_scaling(self,
                          ax: Axes,
                          x_variable: str,
                          y_variable: str,
                          xaxis_log_scale: bool = False,
                          yaxis_log_scale: bool = False,
                          auto_log_scale: bool = True) -> None:
        """
        ...
        if auto_log_scale and self.constants:
            exp_params = getattr(self.constants, 'PARAMETERS_WITH_EXPONENTIAL_FORMAT', set())
    ...
    def setup_axes_limits_and_orientation(self,
                                        ax: Axes,
    ...
    def setup_complete_layout(self,
                             x_variable: str,
                             y_variable: str,
                             ...
                             apply_custom_function: Optional[Callable] = None) -> Tuple[Figure, Axes]:
        """
        ...
        self.setup_axes_labels(ax, x_variable, y_variable, xaxis_label, yaxis_label, font_size)
        self.setup_axes_scaling(ax, x_variable, y_variable, xaxis_log_scale, yaxis_log_scale)
        self.setup_integer_ticks(ax, x_variable, y_variable)
        self.setup_axes_limits_and_orientation(ax, xlim, ylim, xaxis_start_at_zero, 
                                              yaxis_start_at_zero, invert_xaxis, invert_yaxis)
        ...
        # Adjust margins
        self.adjust_margins(fig, left_margin, right_margin, bottom_margin, top_margin)
