...
def critical_mass_optimum_range(bare_mass_values_array, average_squared_effective_mass_estimates_array, sample_size):
  ...
  linear_fit = lsqfit.nonlinear_fit(data=(x, y), p0=linear_fit_p0, fcn=linear_function, debug=True)
  # Quadratic fit
  quadratic_fit_p0 = [gv.mean(linear_fit.p[0]), gv.mean(linear_fit.p[1]), 0.1*gv.mean(linear_fit.p[1])]
  quadratic_fit = lsqfit.nonlinear_fit(data=(x, y), p0=quadratic_fit_p0, fcn=quadratic_function, debug=True)
  ...
  for upper_index_cut in range(minimum_upper_index_cut, total_number_of_data_points):
    ...
    y = average_squared_effective_mass_estimates_array[lower_index_cut:upper_index_cut]
    ...
    t_statistic = gv.mean(difference)/gv.sdev((difference)/np.sqrt(total_number_of_data_points-1))
