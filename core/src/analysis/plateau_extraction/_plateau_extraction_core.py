...
def detect_plateau_region(
    ...
    Args:
        time_series: 1D array of values errors: 1D array of
        uncertainties sigma_threshold: Number of sigma for plateau
        detection min_plateau_size: Minimum number of consecutive points
    ...
    for start in range(min_start, max_end - min_plateau_size + 1):
        for end in range(start + min_plateau_size, min(max_end + 1, n_points + 1)):
            ...
            if np.all(deviations <= sigma_threshold):
                # Calculate quality score (prefer longer, more central
                # plateaus)
                length_score = 1.0 / (end - start)  # Prefer longer
                center = (start + end) / 2
                center_score = (
                    abs(center - n_points / 2) / n_points
                    if search_range.get("prefer_central")
                    else 0
                )
...
def extract_plateau_value_from_samples(
    ...
    Args:
        jackknife_samples: 2D array (samples × time) plateau_bounds:
        (start, end) indices of plateau method: Estimation method
...
def process_single_group(
    ...
    Args:
        jackknife_samples: 2D array of jackknife samples mean_values: 1D
