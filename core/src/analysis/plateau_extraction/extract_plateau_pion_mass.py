...
This script extracts plateau pion effective mass values from effective mass 
time series using jackknife analysis and robust plateau detection methods.

The script processes HDF5 files from calculate_effective_mass.py, detects
plateau regions, and exports results to CSV.

Usage:
    python extract_plateau_pion_mass.py -i effective_mass_analysis.h5 -o output_dir
"""
...
def _apply_preprocessing(
    ...
    if APPLY_SYMMETRIZATION:
        logger.info("Applying symmetrization to pion effective mass data")
...
@click.option(
    ...
    default="plateau_pion_mass_extraction.h5",
    help="Output HDF5 filename for visualization data.",
)
@click.option(
    "-out_csv",
    "--output_csv_filename",
...
def main(
    ...
    """
    Extract plateau pion effective mass values from time series.
    
    This script processes pion effective mass jackknife samples, detects 
    plateau regions, and exports results to both CSV (jackknife averages) 
    and HDF5 (individual sample values) formats.
    """
    ...
    try:
        ...
        _export_to_csv(results, output_csv_path, logger)
        ...
        click.echo(
            ...
            f"   HDF5 results: {output_hdf5_path}\n"
