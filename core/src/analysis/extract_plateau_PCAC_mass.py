...
def _prepare_output_directories(
    ...
    if enable_plotting:
        ...
        plots_dir = os.path.join(output_directory, plot_config['output']['base_directory'])
...
def _process_all_groups(
    ...
    with h5py.File(input_hdf5_file, 'r') as hdf5_file:
        ...
        for group_idx, group_path in enumerate(pcac_groups):
            if verbose:
                click.echo(f"  Processing group {group_idx + 1}/{len(pcac_groups)}: {group_path}")
            
            try:
                result = _process_single_group(
                    hdf5_file, group_path, plots_dir, enable_plotting, logger, verbose
                )
                extraction_results.append(result)
                
                status = "✓" if result['success'] else "✗"
                logger.info(f"Group {group_path}: {status} - {result.get('summary', 'Processed')}")
...
def _process_single_group(
    ...
    config_labels = _load_configuration_labels(group, jackknife_samples.shape[0], logger)
...
def _load_configuration_labels(group: h5py.Group, n_samples: int, logger) -> List[str]:
    ...
    try:
        ...
        if len(config_labels) < n_samples:
            logger.warning(f"Insufficient config labels ({len(config_labels)}) for samples ({n_samples})")
            config_labels.extend([f"config_{i:03d}" for i in range(len(config_labels), n_samples)])
...
def _extract_group_metadata(
    ...
    logger.debug(f"Extracted {len(metadata)} metadata parameters for group {group_path}")
...
def _create_plateau_extraction_plots(
    ...
    for plot_idx in range(n_plots):
        ...
        try:
            ...
            logger.error(f"Failed to create plot {plot_idx} for group {group_name}: {e}")
...
def _create_single_multi_panel_plot(
    ...
    trim_start = data_range.get('trim_start_points', 0) if data_range.get('apply_trimming', False) else 0
    trim_end = data_range.get('trim_end_points', 0) if data_range.get('apply_trimming', False) else 0
    ...
    full_time_index = np.arange(len(pcac_mean)) + 2  # Add time offset (PCAC starts at t=2)
...
def _export_results_to_csv(
    ...
    for result in extraction_results:
        if result['success']:
            ...
            record.update({
                ...
                'plateau_start_time': result['plateau_bounds'][0] + 2,  # Add time offset
                'plateau_end_time': result['plateau_bounds'][1] + 2,
                'plateau_n_points': result['plateau_bounds'][1] - result['plateau_bounds'][0],
            ...
            if result['failed_extractions']:
                failed_labels = [fe['config_label'] for fe in result['failed_extractions']]
    ...
    if not csv_records:
        ...
        empty_df = pd.DataFrame(columns=[
            'group_name', 'plateau_PCAC_mass_mean', 'plateau_PCAC_mass_error',
...
def _report_final_statistics(
    ...
    # Console output
    click.echo(f"\n📊 Extraction Summary:")
    click.echo(f"   Groups: {successful_groups}/{total_groups} successful ({successful_groups/total_groups*100:.1f}%)")
    click.echo(f"   Samples: {successful_samples}/{total_samples} successful ({successful_samples/total_samples*100:.1f}%)")
    ...
    if failed_group_results:
        ...
        if len(failed_group_results) > 5:
            click.echo(f"   ... and {len(failed_group_results) - 5} more (see log file)")
