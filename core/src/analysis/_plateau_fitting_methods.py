...
def _calculate_jackknife_covariance(jackknife_replicas: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def detect_plateau_region(
    ...
    if T < min_plateau_size:
        raise ValueError(f"Time series length {T} is smaller than minimum plateau size {min_plateau_size}")
    ...
    max_end = min(T, T + search_config['max_end_time']) if search_config['max_end_time'] < 0 else T
    ...
    while iteration < max_iterations:
        ...
        if verbose:
            print(f"Iteration {iteration}: range [{current_start}:{current_end}], "
        ...
        for i in range(len(window_means)):
            score = _calculate_outlier_score(
                window_means, window_stds, i, test_method
            )
    ...
    raise ValueError(f"Failed to detect plateau region with sigma threshold {sigma_threshold}")
...
def _test_plateau_consistency(
    ...
    if test_method == "chi_squared":
        ...
        threshold_chi_squared = expected_chi_squared + sigma_threshold * np.sqrt(2 * n_points)
        ...
        p_value = 1.0 - test_statistic / sigma_threshold if test_statistic <= sigma_threshold else 0.0
...
def _calculate_outlier_score(
    ...
    if test_method == "chi_squared":
        ...
        other_means = np.concatenate([
            window_means[:point_index], 
            window_means[point_index + 1:]
        ])
        ...
        return np.abs(window_means[point_index] - median_value) / window_stds[point_index]
...
def estimate_plateau_value(
    ...
    individual_vars = (N - 1) / N * np.sum((plateau_region - individual_means) ** 2, axis=0)
    ...
    if method == "simple":
        ...
        plateau_var = (N - 1) / N * np.sum((replica_means - np.mean(replica_means)) ** 2)
        ...
        correlation_matrix = covariance_matrix / np.outer(individual_stds, individual_stds)
        mask = ~np.eye(n_plateau_points, dtype=bool)
        avg_correlation = np.mean(correlation_matrix[mask]) if n_plateau_points > 1 else 0.0
    ...
    try:
        ...
        for sigma_threshold in detection_config['sigma_thresholds']:
            try:
                ...
                plateau_std = np.std(plateau_values) / np.sqrt(len(plateau_values))  # Standard error
                ...
                logger.debug(f"Sample {config_label}: Plateau extracted with σ={sigma_threshold}")
...
def extract_plateau_from_group(
    ...
    for i, config_label in enumerate(config_labels):
        ...
        plateau_value, plateau_bounds, diagnostics, error_msg = extract_plateau_from_single_sample(
            sample_series, config_label, logger
        )
    ...
    if plateau_bounds_list:
        ...
        logger.debug(f"Group {group_name}: Using common plateau range [{common_start}:{common_end}]")
    else:
        return {
            'success': False,
            'group_name': group_name,
    ...
    try:
        ...
        for fallback in estimation_config['fallback_methods']:
            try:
                ...
                logger.info(f"Group {group_name}: Used fallback method {fallback['method']}")
                break
            except Exception:
                continue
        else:
