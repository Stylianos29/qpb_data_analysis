...
def analyze_and_plot_pion_correlators(hdf5_file_path):
    ...
    grouped_data = defaultdict(lambda: defaultdict(list))  # {(kernel_type, n_value, mscg_eps): {fraction_type: [group_paths]}}
    all_params = []
    ...
    for group_path in analyzer.active_groups:
        ...
        if kernel_type and n_value is not None and mscg_eps is not None and fraction_type:
            key = (kernel_type, n_value, mscg_eps)
            grouped_data[key][fraction_type].append(group_path)
            
        print(f"  {group_path}")
        print(f"    -> Kernel: {kernel_type}, n: {n_value}, MSCG_eps: {mscg_eps}, fraction: {fraction_type}")
        ...
        for fraction_type, group_paths in fraction_groups.items():
            ...
            for group_path in group_paths:
                try:
                    ...
                    print(f"    Warning: Could not extract g5-g5 from {group_path}: {e}")
                    continue
...
def create_pion_correlator_plots(correlator_data):
    ...
    for (kernel_type, n_value, mscg_eps), fraction_data in sorted(correlator_data.items()):
        # Set up plot
        fig, ax = plt.subplots(1, 1, figsize=(12, 8))
        
        print(f"  Plot {plots_created + 1}: Kernel={kernel_type}, n={n_value}, MSCG_eps={mscg_eps}")
            ...
            for j, correlator in enumerate(correlators):
                if hasattr(correlator, '__len__') and len(correlator) > 0:
                    ...
                    ax.plot(x_vals, correlator, color=color, marker=marker, 
                           alpha=0.8, markersize=5, linestyle='None',
                           label=label)
    ...
    for i, group_path in enumerate(sorted(analyzer.active_groups)):
        ...
        try:
            ...
            if 'g5-g5' in datasets:
                g5g5_data = analyzer.dataset_values('g5-g5', return_gvar=False, group_path=group_path)
