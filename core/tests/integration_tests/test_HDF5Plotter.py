...
def analyze_and_plot_correlators(hdf5_file_path):
    ...
    other_multivalued_params = [
        p
        for p in analyzer.reduced_multivalued_tunable_parameter_names_list
        if p != "KL_diagonal_order"
    ]

    grouped_data = analyzer.group_by_multivalued_tunable_parameters(
        filter_out_parameters_list=[
            "KL_diagonal_order",
            "Number_of_gauge_configurations",
        ],  # Exclude KL_diagonal_order from grouping
        verbose=True,
    )

    print(
        f"Found {len(grouped_data)} unique parameter combinations (excluding KL_diagonal_order)"
    )
...
def create_correlator_plots(plot_groups, plot_group_info):
    ...
    for group_key, kl_data in plot_groups.items():
                ...
                if hasattr(correlator, "__len__") and len(correlator) > 0:
                    ...
                    try:
                        ...
                        if isinstance(correlator[0], gvar.GVar):
                            ...
                            shift = 2
                            correlator = correlator[shift : -shift + 1]
                            mean_vals = gvar.mean(correlator)
                            error_vals = gvar.sdev(correlator)
                            x_vals = np.arange(shift, len(mean_vals) + shift)
            ...
            if len(correlators) > 1:
                try:
                    ...
                    if len(stacked.shape) == 2:  # 2D array of correlators
                        ...
                        # Check if gvar
                        if (
                            hasattr(avg_correlator, "__len__")
                            and len(avg_correlator) > 0
                        ):
        ...
        ax.set_ylabel("a$m_{\mathrm{PCAC}}(t)$", fontsize=15)
        ...
        # legend = ax.legend(sorted_handles, sorted_labels, fontsize=13, loc='center', ncols=3)
        # legend.set_title('n=', prop={"size": 14})

        plt.tight_layout()

        # Save plot
        filename = f"PCAC_mass_correlators_by_KL_diagonal_order_{Kernel_operator_type}_{MSCG_epsilon}_{Bare_mass}.png"  # _{plots_created + 1:03d}
...
def inspect_file_structure(hdf5_file_path):
    ...
    print("-" * 30)
    for dataset in analyzer.list_of_output_quantity_names_from_dataframe[
        :10
    ]:  # Show first 10
        if dataset in analyzer.list_of_single_valued_output_quantity_names:
            print(f"  {dataset}: single-valued")
        else:
            count = analyzer.multivalued_columns_count_dictionary.get(
                dataset, "unknown"
            )
